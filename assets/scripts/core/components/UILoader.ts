...
export class UILoader {
    ...
    _parseStaticRes(item: cc.Asset, tag: string) {
        let keys = cc.loader.getDependsRecursively(item) || [];
        for (let index = 0; index < keys.length; index++) {
            const key = keys[index];
            if (cc.loader["_cache"][key]) {
                cc.loader["_cache"][key].uStatic = true;
                cc.loader["_cache"][key].uTag = tag;
            }
        }
        /* 
        if (item instanceof cc.Texture2D) {
            ...
            keys.forEach((key) => {
                let cacheKey = item["_spriteFrames"][key]._textureFilename;
                if (cc.loader["_cache"][cacheKey]) {
                    cc.loader["_cache"][cacheKey].uStatic = true;
                    cc.loader["_cache"][cacheKey].uTag = tag;
                }
            });
    ...
    _parserComponentSprite(node: cc.Node, num: number) {
        ...
        if (sprite.spriteFrame) {
            ...
            cc.log(node.name + ' 的sprite组件没有设置图片!!!!');
        }
    }
